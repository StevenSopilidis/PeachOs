...
void* isr80h_command7_invoke_system_command(struct interrupt_frame* frame) {
    ...
    struct command_argument* root_comman_argument = &arguments[0];
    const char* program_name = root_comman_argument->argument;
    ...
    res = process_inject_arguments(process, root_comman_argument);
    if (res < 0) 
        return ERROR(res);
